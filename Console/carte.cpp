    ...
    if (joueur->getNom() != "IA") {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            while (compteur < 0 || compteur >= joueur->getNbCartesEnMain() || joueur->getCartes(compteur)->getNom() == NomTactique::chasseur_de_tete || compteur == ctarray2[0]) {
                cout << "WARNING: Entrez un numéro de carte valide (entre 0 et " << joueur->getNbCartesEnMain()-1 << ") et ne choissisez pas ChasseurDeTete.Veillez également à ne pas choisir les 2 mêmes cartes. \n";
                cin >> compteur;
            }
            ctarray2[i] = compteur;
        }
...
void TraitreEffet::appliquerEffet() {
    ...
    if (Partie::getInstance().getDernierjoueur()->getNom() != "IA") {
        ...
        //On vérifie que borne est compris entre 0 et 8 pour pouvoir faire les autre tests
        while (borne < 0 || borne > 8) {
            cout << "WARNING: La borne doit être comprise entre 0 et 8 ! \n\n";
            cin >> borne;
        }
        ...
        while (Partie::getInstance().getFrontiere(borne2)->getProprietaire() != nullptr || max_cartes_borne == Partie::getInstance().getFrontiere(borne2)->getnbCartes2()) {
            borne2 = rand() % 9;
