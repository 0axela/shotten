...
bool Partie::VerifGagnantManche(Joueur* j) {
    ...
    Joueur* opponent = (j == joueur1) ? joueur2 : joueur1;
    bool win = false;
    int count = 0;
...
Partie& Partie::getInstance(string gamemode, string nbManches, string nbJoueur, string nomJ1, string nomJ2) {
    if (instanceUnique == nullptr) {
        instanceUnique = new Partie(gamemode, nbManches, nbJoueur, nomJ1, nomJ2);
    }
    return *instanceUnique;
}

Partie::Partie(string gamemode, string nbManches, string nbJoueur, string nomJ1, string nomJ2) : gagnant(nullptr), dernierjoueur(nullptr), currentManche(0) {
    //Initialiser attribut mode
    if (gamemode == "Mode normal") mode = Mode::normal;
    else if (gamemode == "Mode tactique") mode = Mode::tactique;
    else mode = Mode::expert;

    //initialiser nb_manches
    nb_manches = stoi(nbManches.substr(0, 1));


    //Choix IA ou non
    int nb = stoi(nbJoueur.substr(0, 1));
    if (nb == 1){
        joueur1 = new Joueur(1, nomJ1);
        ia = new IA();
        joueur2 = nullptr;
    }
    else if (nb == 2){
