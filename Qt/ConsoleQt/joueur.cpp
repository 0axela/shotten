...
void Joueur::JouerCarte() {
    ...
    bool dejaRevendique = false; //mode expert à ne pas confondre avec le fait que la borne soit déjà revendiquée par un joueur

    if ((Partie::getInstance().getMode() == Mode::expert && dejaRevendique == false)) {
        int reponse;
        cout << "Voulez vous revendiquer une borne ? Tapez 1 pour oui et 2 pour non\n";
        cin >> reponse;
        ...
        if (reponse == 1) { //le joueur veut revendiquer la borne avant de jouer
            size_t indiceBorne1;
            cout << "Choisissez l indice de la borne que vous voulez revendiquer : \n";
            cin >> indiceBorne1;
            while (indiceBorne1 < 0 || indiceBorne1 > 8 || (Partie::getInstance().getFrontiere(indiceBorne1)->getProprietaire() != nullptr)) { //si l'indice de la borne n'est pas correct ou si la borne est déjà revendiquée
                cout << "L'indice de la borne doit être compris entre 0 et 8 et la borne ne doit pas être déjà revendiquée \n";
                cout << "Retapez un indice \n";
                cin >> indiceBorne1;
            }
            Partie::getInstance().getFrontiere(indiceBorne1)->verifRevendication(this);
            //on appelle verifRenvendication avec this qui représente le joueur actuel
        }
        else { //le joueur ne veut pas revendiquer de borne avant de jouer une carte
            cout << "Pas de revendication \n";
        }
        dejaRevendique = true;
    }

    //Affichage de l'aide combinaison
    /*
    int reponseAide;
    cout << "Voulez vous afficher l'aide pour former des combinaisons ? Tapez 1 pour oui et 2 pour non\n";
    cin >> reponseAide;
    while (reponseAide < 1 || reponseAide > 2) { //si la reponse n'est pas 1 ou 2
        cout << "Votre reponse doit etre 1 (pour oui) ou (2 pour non).\n";
        cout << "Retapez une reponse.\n";
        cin >> reponseAide;
    }
    if (reponseAide == 1) {
        afficherAideCombi();
    ...
    */
    ...
    if (Partie::getInstance().getMode() == Mode::tactique || Partie::getInstance().getMode() == Mode::expert) {
        switch (numero) {
            ...
            case 2:
                ...
                if (nb_tactiques_jouees > Partie::getInstance().getJoueur1()->getNb_tactiques_jouee() && clan_dans_main == false) {
                    cout << "\nVous avez déjà joué plus d'une carte tactique de plus que votre adversaire et vous n'avez que des cartes tactiques dans votre jeu, vous ne pouvez donc pas jouer !\n";
                    passer_tour = true;
                }
                break;
    ...
    //Correction: on demande un nouvel indice borne car le joueur n'est pas obligé de choisir la même borne que celle sur laquelle il a joué
    if (Partie::getInstance().getMode() == Mode::normal || Partie::getInstance().getMode() == Mode::tactique ||
        (Partie::getInstance().getMode() == Mode::expert && !dejaRevendique)) {
        int reponse;
        cout << "Voulez vous revendiquer une borne ? Tapez 1 pour oui et 2 pour non\n";
        cin >> reponse;
        while (reponse < 1 || reponse > 2) { //si la réponse n'est pas 1 ou 2
                ...
                case 1:
                    ...
                    for (i = 0; i < nb_cartes_j2; i++) {
                        if (Partie::getInstance().getFrontiere(indiceBorneRevendique)->getCarte2(i)->getNom() == NomTactique::joker ||
                            ...
                            cout << "\n" << this->getNom() << " souhaite revendiquer la borne mais " << Partie::getInstance().getJoueur2()->getNom()
                                 << " a sa carte " << Partie::getInstance().getFrontiere(indiceBorneRevendique)->getCarte2(i)->getNom()
                                 << " à initialiser ! \n";
...
void IA::JouerCarte() {
    ...
    if ((Partie::getInstance().getMode() == Mode::expert && dejaRevendique == false)) {
        ...
        if (reponse == 1) { //l'IA veut revendiquer une borne avant de jouer
            size_t indiceBorne1 = rand() % 9;
            cout << "\nL'IA choisit de tenter de revendiquer la borne n° " << indiceBorne1 << "avant de jouer\n\n";
            Partie::getInstance().getFrontiere(indiceBorne1)->verifRevendication(this);
            //on appelle verifRenvendication avec this qui représente le joueur actuel
        }
        else { //le joueur ne veut pas revendiquer de borne avant de jouer une carte
            cout << "L'IA ne souhtaite pas revendiquer avant de jouer \n";
        }
        dejaRevendique = true;
    }
